...
export const StoreClient = Test.withContext({
    'should store a CAR file': async (assert, { connection, provisionsStorage, storeTable }) => {
    ...
    'should list stored CARs': async (assert, { connection, provisionsStorage, storeTable }) => {
        const alice = new Client(await AgentData.create(), {
            // @ts-ignore
            serviceConf: {
                access: connection,
        ...
        const { results: [entry], } = await alice.capability.store.list();
        assert.deepEqual(entry.link, car.cid);
        assert.deepEqual(entry.size, car.size);
    },
    'should remove a stored CAR': async (assert, { connection, provisionsStorage }) => {
        const alice = new Client(await AgentData.create(), {
            // @ts-ignore
            serviceConf: {
                access: connection,
    ...
    'should get a stored item': async (assert, { connection, provisionsStorage }) => {
        const car = await randomCAR(128);
        const alice = new Client(await AgentData.create(), {
            // @ts-ignore
            serviceConf: {
