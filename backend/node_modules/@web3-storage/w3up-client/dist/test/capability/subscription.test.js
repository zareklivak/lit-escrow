...
export const SubscriptionClient = Test.withContext({
    list: {
        'should list subscriptions': async (assert, { client, connection, service, plansStorage, grantAccess, mail }) => {
            ...
            await account.save();
            assert.deepEqual(await client.capability.subscription.list(account.did()), { results: [] });
            const result = await account.provision(space.did());
            assert.ok(result.ok);
            assert.deepEqual(await client.capability.subscription.list(account.did(), {
                nonce: 'retry',
            }), {
                results: [
                    {
